...
class ExportadorPDF(Exportador):
    ...
    def exportar(self, raiz: VirtualFolder, caminho_saida: Path | None = None) -> None:
        ...
        if not _FPDF_AVAILABLE:
            raise ImportError("fpdf2 não está instalado. Instale com: pip install fpdf2")

        pdf: FPDF = FPDF()  # type: ignore
        pdf.add_page()
    ...
    @staticmethod
    def _montar_celula_pdf(pdf: Any, bm: TagA) -> None:
