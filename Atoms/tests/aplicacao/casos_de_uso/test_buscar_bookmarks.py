...
Verifica a busca de arquivos HTML de bookmarks, o processamento de metadados
...
def test_processar_arquivo_sucesso(arquivo_valido: Path) -> None:
    """Processa um arquivo HTML com um link e retorna metadados de sucesso."""
    meta: dict[str, str | int | None] = processar_arquivo(arquivo=arquivo_valido)
    assert meta["status"] == "sucesso"
    assert meta["nome"] == arquivo_valido.name
    assert meta["itens_raiz"] == 1
...
def test_processar_arquivo_sem_dl(arquivo_sem_dl: Path) -> None:
    """Retorna erro quando a tag <DL> não é encontrada no HTML."""
    meta: dict[str, str | int | None] = processar_arquivo(arquivo=arquivo_sem_dl)
    assert meta["status"] == "erro"
    assert meta["erro"] == "Tag <DL> raiz não encontrada"


def test_processar_arquivo_inexistente() -> None:
