...
class TestNormalizarEValidar:
    ...
    def test_diretorio_existente_retorna_path_resolvido(self, tmp_path: Path) -> None:
        ...
        assert normalizar_e_validar(caminho_bruto=str(tmp_path)) == tmp_path.resolve()
...
class TestConfirmarDadosEntrada:
    ...
    def test_retorna_lista_com_diretorio_valido(self, tmp_path: Path) -> None:
        ...
        resultado: list[Path] = confirmar_dados_entrada(caminhos=tmp_path)
        # aceitar Paths ou strings retornados pela função; comparar como Paths resolvidos
        resultado_resolvido: list[Path] = [Path(p).resolve() for p in resultado]
        assert resultado_resolvido == [tmp_path.resolve()]
    ...
    def test_diretorio_invalido_levanta_erro_com_contexto(self, tmp_path: Path) -> None:
        ...
        contexto: dict[str, object] = exc_info.value.contexto
        caminhos_tentados: object = contexto.get("caminhos_tentados", contexto.get("caminhos"))
        assert caminhos_tentados is not None

        if isinstance(caminhos_tentados, (list, tuple)):
            assert any(Path(caminho) == invalido for caminho in caminhos_tentados)
        else:
            assert Path(str(caminhos_tentados)) == invalido
...
class TestLerArquivoHtml:
    ...
    def test_le_conteudo_de_arquivo_existente(self, tmp_path: Path) -> None:
        ...
        assert ler_arquivo_html(caminho=arquivo).strip() == "<html>conteúdo</html>"
