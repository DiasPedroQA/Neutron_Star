...
def test_criacao_com_todos_os_campos_preserva_os_valores() -> None:
    """Garante que todos os campos explícitos são armazenados e sanitizados."""
    favorito = Favorito(
        titulo="  GitHub  ",
...
def test_campos_vazios_ou_com_espacos_recebem_defaults_no_objeto() -> None:
    """Garante que strings vazias ou só espaços são normalizadas no próprio objeto."""
    favorito = Favorito(
        titulo="   ",
        url="https://github.com",
        pasta="",
...
def test_to_dict_com_todos_os_campos_preenchidos() -> None:
    """Garante o mapeamento correto de campos para as chaves PascalCase esperadas no dict."""
    favorito = Favorito(
        titulo="GitHub",
        url="https://github.com",
        pasta="Trabalho / Dev",
